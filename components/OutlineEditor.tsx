...
const OutlineEditor: React.FC<OutlineEditorProps> = ({ topic, initialAttachments, onBack, onGenerateSlides }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-zinc-950">
      {/* Navbar */}
      <header className="h-16 border-b border-zinc-800 flex items-center px-6 justify-between bg-zinc-950/50 backdrop-blur">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="p-2 hover:bg-zinc-900 rounded-full text-zinc-400 hover:text-white transition">
            <ArrowLeft size={20} />
          </button>
          <div className="flex flex-col">
            <h2 className="font-semibold text-lg truncate max-w-md">{topic || "Untitled"}</h2>
        ...
        <div className="flex items-center gap-3">
            {isGenerating && <span className="text-xs text-purple-400 animate-pulse flex items-center gap-1"><Loader2 size={12} className="animate-spin"/> AI Generating...</span>}
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-1/3 border-r border-zinc-800 bg-black p-6 overflow-y-auto hidden md:block">
          <h3 className="text-zinc-500 text-xs font-mono uppercase tracking-widest mb-4">Live Inference Stream</h3>
          <pre className="font-mono text-xs text-green-500/80 whitespace-pre-wrap break-all leading-relaxed">
        ...
        <div className="flex-1 bg-zinc-950 p-6 overflow-y-auto">
          <div className="max-w-3xl mx-auto space-y-6 pb-20">
            <div className="flex justify-between items-end mb-6">
                <div>
                    <h3 className="text-2xl font-bold">Slide Outline</h3>
                    <p className="text-zinc-400 text-sm mt-1">Review and edit your structure before generating images.</p>
                </div>
            </div>
            ...
            {slides.map((slide, index) => (
              <div key={slide.id} className="group bg-zinc-900 border border-zinc-800 rounded-xl p-6 transition-all hover:border-zinc-700 hover:shadow-xl hover:shadow-black/50">
                <div className="flex justify-between items-start mb-4 border-b border-zinc-800 pb-2">
                  <span className="text-xs font-mono text-zinc-500 bg-zinc-950 px-2 py-1 rounded">SLIDE {index + 1}</span>
                  <button 
                ...
                <div className="space-y-4">
                  ...
                  <div>
                     <label className="text-xs text-zinc-500 uppercase font-semibold flex items-center gap-2">
                        Visual Prompt <span className="text-[10px] bg-purple-900/30 text-purple-300 px-1 rounded">AI</span>
                     </label>
                     <textarea 
